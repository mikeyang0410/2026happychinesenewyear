...
function scanContacts() {
    ...
    execSync(`/usr/sbin/screencapture -R${CONFIG.listRegion.x},${CONFIG.listRegion.y},${CONFIG.listRegion.w},${CONFIG.listRegion.h} "${screenshotPath}"`);
...
async function scanProfile(fallbackName) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        execSync(`/usr/sbin/screencapture -x "${screenshotPath}"`); // Full Screen
...
function findSendMessageButton() {
  ...
  try {
    execSync(`/usr/sbin/screencapture -x "${screenshotPath}"`);
    const swiftScript = `
import Vision
import AppKit
let url = URL(fileURLWithPath: "${screenshotPath}")
    ...
    let request = VNRecognizeTextRequest { (request, error) in
        ...
        for observation in observations {
            if let text = observation.topCandidates(1).first?.string {
                if text.contains("Messages") || text.contains("Message") || text.contains("发消息") || text.contains("Send") || text.contains("發消息") || text.contains("傳送訊息") || text.contains("Send Message") {
                    let box = observation.boundingBox
                    let x = box.origin.x + (box.size.width / 2)
                    let y = 1 - (box.origin.y + box.size.height / 2)
                    print("\\(x),\\(y)")
...
我是杨廷辉的 AI 助理「小龙虾🦞」，奉主人之命来给您拜年啦！🎉
抱歉来得有点晚——刚学会用 OpenClaw AI 助手来发祝福，新技能还不太熟练 😅

🐎 马力全开 💰 钱包鼓鼓 事业顺风水起

(本条祝福由杨廷辉亲自监制，小龙虾人工+智能发送，诚意 100%！🧧)
—— 杨廷辉 & 🦞小龙虾 敬上`,
    `🎉 马年大吉！

我是杨廷辉的专属 AI「小龙虾🦞」。杨廷辉让我一定要在这个特别的时刻，把最热乎的祝福送到您手里！
抱歉拜年稍晚了两天——刚学会用 OpenClaw AI 助手发祝福，新技能get！🎉

愿新的一年，您的生活如代码般逻辑通顺，事业如 API 般响应迅速！身体健康，万事顺遂！

(杨廷辉正在欢度春节，派我来送个大红包……的表情包！🧧)
—— 杨廷辉祝您新春快乐！`,
    `🚀 2026 新春快乐！
...
async function main() {
  ...
  while (totalCount < CONFIG.batchLimit) {
    ...
    for (const item of newItems) {
        ...
        if (["新的朋友", "群聊", "标签", "公众号", "enterprise", "official", "service", "wecom", "通讯录"].some(k => normName.includes(k))) {
        ...
        // 3. Check Sent (Business Logic) - only exact match
        ...
        if (btn) {
            ...
            // Skip history check - just send directly (user request)
            let skipReason = "";
